...
async function main() {
  ...
  const tx1 = await PredictionMarket.connect(user1).createMarket(
    "World Cup 2026: Which side wins the opening match?",
    "Resolves to the full-time winner of the opening match",
    ["Home", "Away"],
    shortResolution,
    arbitrator.address,
    ethers.ZeroAddress
  ...
  const tx2 = await PredictionMarket.connect(user1).createMarket(
    "World Cup 2026: Which team wins Group A?",
    "Resolves to the team officially ranked first in Group A",
